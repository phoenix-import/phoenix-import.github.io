...
- Console-script pushes NL via `productUpdate`, then non-NL via
  ...
  storefront domain.
- Paragon: a product file (needs `status=Active`), then an associations file
  (one `Vertaling` row per locale).
- **Output filenames** are auto-generated by `exportName(prefix, ext)`:
  `<prefix>_<first word of the first titled product>_<DDMMYYYY>` — prefixes
  `translate`, `proofread`, `PRG_product`, `PRG_associated`. E.g. a first title of
  "Tingsha's Flower of life -- 7cm" → `translate_tingsha_29062026.csv`. Possessive
  `'s` is dropped, non-alphanumerics stripped, empty falls back to `export`.
